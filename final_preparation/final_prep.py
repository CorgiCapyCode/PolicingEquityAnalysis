...
from feature_selec.feature_selection import chi_square_test, chi_square_test_label_encoder, chi_square_test_one_hot_encoder
from sklearn.metrics import adjusted_mutual_info_score
from sklearn.preprocessing import LabelEncoder, MinMaxScaler



def feature_final_preparation(df: pd.DataFrame, run_all: bool =True):
    ...
    if run_all:
        mutual_comparison_results, chi_comparison_results = feature_comparison(df=df)
        plot_comparison_results(df=mutual_comparison_results)
    else: 
        mutual_comparison_results = None
        chi_comparison_results = None
      
    return mutual_comparison_results, chi_comparison_results
...
def feature_comparison(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mutual_comparison_results = pd.DataFrame(index=feature_names, columns=feature_names)
    ...
    for i in range(len(feature_names)):
        ...
        for j in range(i+1, len(feature_names)):
            ...
            print(f"For feature {feature_1} and feature {feature_2} the max. Chi square value is {final_p_value}")
            mutual_comparison_results.loc[feature_2, feature_1] = mutual_value
    ...
    chi_comparison_results = pd.DataFrame(results)
