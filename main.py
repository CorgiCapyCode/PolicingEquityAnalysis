...
import pandas as pd
from metadata_analysis.metadata import save_df_to_csv
...
def check_for_features_and_add_to_dataframe(df: pd.DataFrame, df_dict: Dict[str, pd.DataFrame]):
...
def create_dataframe(feature_list: list, department_names: list) -> pd.DataFrame:
...
def main():
    ...
    feature_analysis_dataframe = create_dataframe(feature_list=unique_feature_list, department_names=department_names)   
    # print(feature_analysis_dataframe)
    
    check_for_features_and_add_to_dataframe(df=feature_analysis_dataframe, df_dict=df_dictionary)
    # print(feature_analysis_dataframe)
    feature_analysis_df_with_sum = add_sum_of_usages(feature_analysis_dataframe)
    # print(feature_analysis_df_with_sum)
    save_df_to_csv(df=feature_analysis_df_with_sum, output_filename="feature_analysis_dataframe.csv")
    # endregion
    # region - Check police reports for reports using the same features
    
    corr_matrix = create_department_correlation_matrix(feature_analysis_dataframe)
    
    plot_and_save_heatmap(correlation_matrix=corr_matrix, save_name="original_department_correlation")
