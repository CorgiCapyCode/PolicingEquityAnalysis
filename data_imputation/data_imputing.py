...
def impute_data(df: pd.DataFrame, show_results: bool =False):
  ...
  prob_distribution_veh_make = complex_probabilistic_imputer(df=df, feature_name=("VEHICLE_MAKE", "VEH_MAKE"), not_considered_values=not_considered_list)
  prob_distribution_veh_year = complex_probabilistic_imputer(df=df, feature_name=("VEHICLE_YEAR", "VEH_YEAR_NUM"), not_considered_values=not_considered_list)
  prob_distribution_veh_color = complex_probabilistic_imputer(df=df, feature_name=("VEHICLE_COLOR", "VEH_COLOR"), not_considered_values=not_considered_list)
  prob_distribution_veh_occ = complex_probabilistic_imputer(df=df, feature_name=("VEHICLE_DETAILS", "VEH_OCCUPANT"), not_considered_values=not_considered_list)
  prob_distribution_veh_state = complex_probabilistic_imputer(df=df, feature_name=("VEHICLE_DETAILS.1", "VEH_STATE"), not_considered_values=not_considered_list)
  ...
  _ = simple_probabilistic_imputer(df=df, feature_name=("SUBJECT_RACE", "DESCRIPTION"))
  _ = simple_probabilistic_imputer(df=df, feature_name=("SUBJECT_DETAILS.2", "COMPLEXION"))  
  ...
  _ = simple_probabilistic_imputer(df=df, feature_name=("OFFICER_ID", "OFFICER_ID"))
  _ = simple_probabilistic_imputer(df=df, feature_name=("OFFICER_SUPERVISOR", "SUPERVISOR_ID"))
  
  
  multivariate_bayesian_imputer(df=df, target_feature=("LOCATION_CITY", "CITY"), cond_feature=("LOCATION_STREET_NUMBER", "STREET_ID"))
  _ = simple_probabilistic_imputer(df=df, feature_name=("LOCATION_CITY", "CITY"))
  ...
  if show_results:
    ...
    print("*******************")
    print("")
    print(f"The probability distribution for feature ('SUBJECT_GENDER', 'SEX') is:")
    print(prob_distribution_sex)
    print(f"The probability distribution for feature ('INCIDENT_DATE', 'FIO_DATE') is:")
    ...
    print(f"The probability distribution for feature ('OFFICER_AGE', 'AGE_AT_FIO_CORRECTED') is:")
    print(prob_distribution_age)
    
    print(f"The probability distribution for feature ('VEHICLE_MAKE', 'VEH_MAKE') is:")
    print(prob_distribution_veh_make)    
    print(f"The probability distribution for feature ('VEHICLE_YEAR', 'VEH_YEAR_NUM') is:")
    print(prob_distribution_veh_year)    
    print(f"The probability distribution for feature ('VEHICLE_COLOR', 'VEH_COLOR') is:")
    print(prob_distribution_veh_color)    
    print(f"The probability distribution for feature ('VEHICLE_DETAILS', 'VEH_OCCUPANT') is:")
    print(prob_distribution_veh_occ)    
    print(f"The probability distribution for feature ('VEHICLE_DETAILS.1', 'VEH_STATE') is:")
    print(prob_distribution_veh_state)
  
  
        
def simple_probabilistic_imputer(df: pd.DataFrame, feature_name: str):
...
def complex_probabilistic_imputer(df: pd.DataFrame, feature_name: str, not_considered_values: list):
  # Ensure reproducibility
  np.random.seed(42)
